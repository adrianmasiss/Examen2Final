    ...
    const responder = async (e) => {
        ...
        try {
            const res = await fetch(`/api/preguntas/${pregunta.id}/responder`, {
                ...
                body: JSON.stringify({ opcionId: seleccionada })
            });
            const data = await res.json();
            setFeedback(data.correcta);

            // Encuentra el texto de la opción seleccionada para mostrar en historial
            const textoOpcion = pregunta.opciones.find(op => op.id === seleccionada)?.texto || "";
            if (onRespondido) onRespondido({ ...data, opcionTexto: textoOpcion });

        } catch (err) {
            setFeedback(false);
            if (onRespondido) onRespondido({ correcta: false, opcionTexto: "" });
        }
    };
